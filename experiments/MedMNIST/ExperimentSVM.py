...
class ExperimentSVM(ExperimentMedMNISTBase):
    '''
    ...
    def __init__(self, known_anomalies, pollution, seed=None, **kwargs) -> None:
        ...
        self.experiment = f'SVM/medmnist_pollution_{pollution}'
    ...
    def __optimize_svm__(self, verbose) -> dict:
        ...
        x_train, y_train = np.stack(x_train), np.array(y_train).flatten()
    ...
    def __get_random_test_subset__(self, percent=.1):
        ...
        x_test, y_test = np.stack(x_test), np.array(y_test).flatten()
    ...
    def __get_test_data__(self) -> tuple:
        ...
        y = self.__change_labels__(np.array(y).flatten())
        return (X, y)